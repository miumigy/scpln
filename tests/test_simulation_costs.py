...
def test_store_sales_record_sgna_cost():
    product = Product(
        name="P1", sales_price=200.0, unit_cost=80.0, sgna_cost_per_unit=12.5
    )
    store = StoreNode(name="S1", initial_stock={"P1": 100}, lead_time=0)
    demand = CustomerDemand(
        store_name="S1",
    ...
    total_sgna_cost = sum(pl.get("sgna_cost", 0.0) for pl in daily_pl)
    assert total_sgna_cost == pytest.approx(4 * 10.0 * 12.5)

    assert any(evt.get("account") == "sgna" for evt in sim.cost_trace)
