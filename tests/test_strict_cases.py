...
class TestStrictCases(unittest.TestCase):
    def test_warehouse_orders_include_scheduled_outgoing(self):
        ...
        sim_input = {
            "planning_horizon": 2,
            "products": [
                {"name": "完成品A", "sales_price": 1000, "assembly_bom": []}
            ],
            "nodes": [
                ...
                {"name": "組立工場", "node_type": "factory", "producible_products": ["完成品A"],
                 "initial_stock": {"完成品A": 0}, "lead_time": 1, "production_capacity": 100000, "backorder_enabled": True},
            ],
            "network": [
        ...
        s_to_w = sum(q for item, q, src, dest in day0_orders if (item, src, dest) == ("完成品A", "中央倉庫", "店舗1")) \
                 + sum(q for item, q, src, dest in day0_orders if (item, src, dest) == ("完成品A", "中央倉庫", "店舗2"))
        ...
        # qty_wh = 30 - (-75) = 105
        w_to_f = sum(q for item, q, src, dest in day0_orders if (item, src, dest) == ("完成品A", "組立工場", "中央倉庫"))
    ...
    def test_lcm_order_multiple_rounding(self):
        ...
        sim_input = {
            ...
            "nodes": [
                {"name": "店舗1", "node_type": "store", "initial_stock": {"完成品A": 0}, "service_level": 0.0,
                 "order_multiple": {"完成品A": 4}},
                {"name": "中央倉庫", "node_type": "warehouse", "initial_stock": {"完成品A": 0}, "service_level": 0.0},
            ],
            "network": [
                {"from_node": "中央倉庫", "to_node": "店舗1", "lead_time": 3, "order_multiple": {"完成品A": 6}},
            ],
            "customer_demand": [
                {"store_name": "店舗1", "product_name": "完成品A", "demand_mean": 10, "demand_std_dev": 0}
            ],
        ...
        sim.run()
        day0_orders = sim.order_history.get(0, [])
        store_order = sum(q for item, q, src, dest in day0_orders if (item, src, dest) == ("完成品A", "中央倉庫", "店舗1"))
