...
def test_dockerfile_exists_and_has_expected_content():
    assert DOCKERFILE.exists(), "Dockerfile が存在しません（リポジトリ直下に作成してください）"
...
@pytest.mark.skipif(shutil.which("docker") is None, reason="docker コマンドが見つからないためスキップ")
def test_docker_image_builds(tmp_path):
    ...
    try:
        ...
        subprocess.run(["docker", "rmi", "-f", tag], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
