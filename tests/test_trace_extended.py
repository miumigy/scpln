...
def _payload_store_only(days=2):
    ...
    return SimulationInput(
        ...
        customer_demand=[
            CustomerDemand(store_name="S1", product_name="P1", demand_mean=20.0, demand_std_dev=0.0)
        ],
        random_seed=1,
    )
...
def test_trace_amounts_match_pl_components_for_store_storage_and_penalties():
    ...
    total_store_storage_fixed = sum(pl["stock_costs"]["store_storage_fixed"] for pl in daily_pl_list)
    total_store_storage_var   = sum(pl["stock_costs"]["store_storage_variable"] for pl in daily_pl_list)
    total_penalty_stockout    = sum(pl["penalty_costs"]["stockout"] for pl in daily_pl_list)
    total_penalty_backorder   = sum(pl["penalty_costs"]["backorder"] for pl in daily_pl_list)
