...
from typing import defaultdict as _defaultdict  # type: ignore
...
class SupplyChainSimulator:
                    ...
                    for item_name in items_to_manage:
                        ...
                        profile = (
                            ...
                            if isinstance(current_node, WarehouseNode)
                                ...
                                (d for d in self.input.customer_demand if d.store_name == node_name and d.product_name == item_name),
                                None,
                            )
                        )
                        if not profile:
                ...
                if isinstance(current_node, FactoryNode):
                    ...
                    for fg_item in current_node.producible_products:
                        ...
                        if qty_to_produce > 0:
                            ...
                            self.production_orders[completion_day].append((fg_item, qty_to_produce, node_name))

                    # Component replenishment planning (Factory -> Material suppliers)
                    for item_name, reorder_point in getattr(current_node, "reorder_point", {}).items():
                            ...
                            if qty_to_order > 0:
                                parent_name = next(
                                    (
                                        ...
                                        and self.nodes_map[l.from_node].node_type == "material"
                                        and item_name in self.nodes_map[l.from_node].material_cost
                                    ),
                                    None,
                                )
                                if parent_name:
    ...
    def compute_summary(self):
        ...
        totals_by_type = {t: {"demand": 0.0, "sales": 0.0, "shortage": 0.0, "end_stock_sum": 0.0} for t in ("store", "warehouse", "factory", "material")}
        ...
        avg_on_hand_by_type = {t: totals_by_type[t]["end_stock_sum"] / days for t in totals_by_type}
        ...
        summary = {
            ...
            "network_shortage_total": sum(totals_by_type[t]["shortage"] for t in ("warehouse", "factory", "material")),
        ...
        for key, data in events.items():
            if key.startswith("transport:"):
                ...
                if isinstance(supplier, MaterialNode) and isinstance(dest, FactoryNode):
                    ...
                    pl["material_cost"] += getattr(supplier, "material_cost", {}).get(item, 0) * qty
