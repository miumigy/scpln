...
class Node(BaseModel):
    ...
    reorder_point: Dict[str, float] = Field(default_factory=dict) # {item_name: quantity}
    order_up_to_level: Dict[str, float] = Field(default_factory=dict) # {item_name: quantity}
...
class SupplyChainSimulator:
    def __init__(self, sim_input: SimulationInput):
        ...
        self.in_transit_orders = defaultdict(list) # {arrival_day: [(item_name, quantity, destination_node)]}
        self.daily_results = []
    ...
    def run(self):
        for day in range(self.input.planning_horizon):
            daily_demand_from_children = defaultdict(lambda: defaultdict(float)) # Demand passed up from children
            ...
            for item_name, quantity, dest_node_name in self.in_transit_orders.pop(day, []):
            ...
            for demand_event in self.input.customer_demand:
                demand_qty = max(0, round(random.gauss(demand_event.demand_mean, demand_event.demand_std_dev)))
                daily_demand_from_children[demand_event.store_name][demand_event.product_name] += demand_qty
            ...
            for node_name in self.node_order:
                ...
                items_to_process = set(daily_demand_from_children[node_name].keys()) | set(self.stock[node_name].keys())
                ...
                for item_name in list(items_to_process):
                    ...
                    if demand_qty > 0:
                        daily_events[f'{node_name}_{item_name}']['demand'] += demand_qty # ADDED THIS LINE
                        ...
                        if fulfilled_qty > 0:
                            ...
                            daily_events[f'{node_name}_{item_name}']['sales'] += fulfilled_qty
                        
                        if shortage_qty > 0:
                            daily_events[f'{node_name}_{item_name}']['shortage'] += shortage_qty

                    # --- Production (for Factory Nodes) ---
                    if isinstance(current_node, FactoryNode) and item_name in current_node.producible_products:
                        ...
                        if product_def:
                            ...
                            for arrival_day, orders in self.in_transit_orders.items():
                                for order_item_name, order_qty, dest_node_name in orders:
                                    if dest_node_name == node_name and order_item_name == item_name:
                                        current_inventory_position += order_qty

                            production_needed_for_replenishment = 0
                            target_level = current_node.order_up_to_level.get(item_name, 0)
                            if target_level > 0 and current_inventory_position < target_level:
                                production_needed_for_replenishment = target_level - current_inventory_position

                            # Total production target: demand + replenishment needed
                            production_target = demand_qty + production_needed_for_replenishment
                            
                            can_produce = min(production_target, current_node.production_capacity)
                            
                            # Check if components are available for production
                            components_available = True
                            for bom_item in product_def.assembly_bom:
                                if self.stock[node_name].get(bom_item.item_name, 0) < bom_item.quantity_per * can_produce:
                            ...
                            if components_available and can_produce > 0:
                                ...
                                for bom_item in product_def.assembly_bom:
                                    ...
                                    self.stock[node_name][bom_item.item_name] -= consumed_qty
                                    daily_events[f'{node_name}_{bom_item.item_name}']['consumption'] += consumed_qty
                                
                                # Add produced item to stock
                                self.stock[node_name][item_name] += can_produce
                                daily_events[f'{node_name}_{item_name}']['produced'] += can_produce
                    ...
                    qty_to_order = 0
                    reorder_point = current_node.reorder_point.get(item_name, -1) # -1 means no reorder point
                    order_up_to_level = current_node.order_up_to_level.get(item_name, -1) # -1 means no order up to level

                    if reorder_point != -1 and current_inventory_position <= reorder_point:
                        if order_up_to_level != -1:
                            ...
                            qty_to_order = current_node.moq.get(item_name, 1) # Use MOQ as fixed order quantity if no S

                    if qty_to_order > 0:
                        parent_node_names = self.input.network.get(node_name)
                        if parent_node_names:
                            if isinstance(parent_node_names, str): parent_node_names = [parent_node_names]
                            ...
                            for parent_name in parent_node_names:
                                ...
                                if item_name in self.nodes_map[parent_name].initial_stock or \
                                   (isinstance(self.nodes_map[parent_name], FactoryNode) and item_name in self.nodes_map[parent_name].producible_products):
                                    
                                    # Pass demand upstream to the parent
                                    daily_demand_from_children[parent_name][item_name] += qty_to_order
                                    self._place_order(parent_name, node_name, item_name, qty_to_order, day, daily_events)
                                    supplier_found = True
                                    break # Assume one supplier for now
                            # If no supplier found, the order is lost (or backlogged, not implemented yet)
                            if not supplier_found:
                                daily_events[f'{node_name}_{item_name}']['lost_order'] += qty_to_order
    ...
    def _place_order(self, supplier_node_name: str, customer_node_name: str, item_name: str, quantity: float, current_day: int, daily_events: defaultdict):
        supplier_node = self.nodes_map[supplier_node_name]
        arrival_day = current_day + supplier_node.lead_time
        self.in_transit_orders[arrival_day].append((item_name, quantity, customer_node_name))
        daily_events[f'{customer_node_name}_{item_name}']['ordered_quantity'] += quantity
