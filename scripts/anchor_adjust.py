...
def main() -> None:
    ...
    ap.add_argument("-i", "--inputs", nargs=2, required=True, help="aggregate.json と sku_week.json")
    ap.add_argument("-o", "--output", required=True, help="調整後のDET JSON 出力パス")
    ap.add_argument("--cutover-date", dest="cutover_date", required=True, help="境界日 YYYY-MM-DD（必須）")
    ap.add_argument("--anchor-policy", dest="anchor_policy", default="DET_near", help="anchorポリシー（既定: DET_near）")
    ap.add_argument("--recon-window-days", dest="recon_window_days", type=int, default=None, help="整合ウィンドウ日数（任意）")
    ap.add_argument("--weeks", dest="weeks_per_period", type=int, default=None, help="1期間の週数ヒント（任意）")
    ap.add_argument("--calendar-mode", dest="calendar_mode", default="simple", choices=["simple", "iso"], help="cutover週の推定モード")
    ap.add_argument("--max-adjust-ratio", dest="max_adjust_ratio", type=float, default=None, help="1行あたりの最大相対調整率（例: 0.2=±20%）。未指定で制限なし")
    ap.add_argument("--tol-abs", dest="tol_abs", type=float, default=0.0, help="(family,period)単位の調整スキップ閾値（絶対）。max(|Δ|)≤tolなら調整しない")
    ap.add_argument("--tol-rel", dest="tol_rel", type=float, default=0.0, help="(family,period)単位の調整スキップ閾値（相対）。|Δ|/max(|AGG|,|DET|,1)≤tolなら調整しない")
    ap.add_argument("--carryover", dest="carryover", default="none", choices=["none", "prev", "next", "auto", "both"], help="残差の隣接periodへの持ち越し方向")
    ap.add_argument("--carryover-split", dest="carryover_split", type=float, default=None, help="both/auto時のnext配分率（0..1）。未指定時はpolicy依存の既定(DET_near/blend=0.8, AGG_far=0.2)")
    ap.add_argument("-I", "--input-dir", dest="input_dir", default=None, help="入力CSVディレクトリ（capacity.csvを参照）")
    ap.add_argument("--capacity", dest="capacity_csv", default=None, help="capacity.csv のパス（任意）")
    ap.add_argument("--open-po", dest="open_po_csv", default=None, help="open_po.csv のパス（任意）")
    ap.add_argument("--headroom-inbound-weight", dest="headroom_inbound_weight", type=float, default=0.0, help="隣接period余地の入荷バイアス（0..1推奨）")
    ap.add_argument("--headroom-capacity-weight", dest="headroom_capacity_weight", type=float, default=0.5, help="隣接period余地の容量バイアス（0..1推奨）")
    ap.add_argument("--period-score", dest="period_score_csv", default=None, help="period別の追加スコアCSV（period,score）")
    ap.add_argument("--headroom-score-weight", dest="headroom_score_weight", type=float, default=0.0, help="追加スコアのバイアス係数（0..1推奨）")
    ap.add_argument("--pl-cost", dest="pl_cost_csv", default=None, help="period別のコストCSV（period,cost）")
    ap.add_argument("--headroom-cost-weight", dest="headroom_cost_weight", type=float, default=0.0, help="コストに対する抑制係数（0..1推奨）")
    args = ap.parse_args()
    ...
    for r in det_rows:
        ...
        if not fam or not wk:
            continue
        per = str(r.get("period")) if r.get("period") is not None else _period_from_week(wk)
    ...
    def _indices_for_rows(rows_in: List[Dict[str, Any]]) -> List[int]:
        ...
        for rr in rows_in:
            try:
                ...
                sku = rr.get("sku")
                wk = rr.get("week")
                i = next((k for k, rec in enumerate(det_rows_out) if ((rec.get("family") or rec.get("item")) == fam and rec.get("sku") == sku and rec.get("week") == wk)), -1)
            if i >= 0:
                idxs.append(i)
        return idxs
    ...
    def _load_capacity_map(input_dir: str | None, path: str | None) -> Tuple[Dict[str, float], float]:
        cap: Dict[str, float] = {}
        maxcap = 0.0
    ...
    def _load_inbound_map(input_dir: str | None, path: str | None) -> Tuple[Dict[str, float], float]:
    ...
    def _load_period_value_map(path: str | None, input_dir: str | None, fname: str, col: str) -> Tuple[Dict[str, float], float]:
        m: Dict[str, float] = {}
        mx = 0.0
    ...
    ps_map, ps_max = _load_period_value_map(args.period_score_csv, args.input_dir, "period_score.csv", "score")
    cost_map, cost_max = _load_period_value_map(args.pl_cost_csv, args.input_dir, "period_cost.csv", "cost")
    ...
    def _headroom_for_period(fam: str, per: str, resid_map: Dict[str, float]) -> float:
        ...
        try:
            target = agg_map.get((fam, per)) or {"demand": 0.0, "supply": 0.0, "backlog": 0.0}
            ...
            try:
                if cap_map and per in cap_map and cap_max > 0 and args.headroom_capacity_weight:
                    norm = _cap_norm(per)
                    total *= (1.0 + max(0.0, float(args.headroom_capacity_weight)) * norm)
                if inb_map and per in inb_map and inb_max > 0 and args.headroom_inbound_weight:
                    inb_norm = max(0.0, inb_map.get(per, 0.0) / inb_max)
                    total *= (1.0 + max(0.0, float(args.headroom_inbound_weight)) * inb_norm)
                if ps_map and per in ps_map and ps_max > 0 and args.headroom_score_weight:
                    sc_norm = max(0.0, ps_map.get(per, 0.0) / ps_max)
                    total *= (1.0 + max(0.0, float(args.headroom_score_weight)) * sc_norm)
                if cost_map and per in cost_map and cost_max > 0 and args.headroom_cost_weight:
    ...
    for (fam, per), rows in by_fp_weeks.items():
        ...
        for rr in rows:
            try:
                i = det_rows_out.index(rr)  # rely on object identity; fallback if needed
            except ValueError:
                # find by matching a few keys
                i = next(
                    (k for k, rec in enumerate(det_rows_out) if rec is rr or (
                        rec.get("family") == rr.get("family") and rec.get("sku") == rr.get("sku") and rec.get("week") == rr.get("week")
                    )),
                    -1,
                )
            if i >= 0:
        ...
        try:
            ...
            if args.calendar_mode == "iso":
                ...
                cutover_week_num_est = int(round(frac * max(0, len(label_wk_nums) - 1))) + 1
            else:
        ...
        if args.carryover and args.carryover.lower() != "none":
            ...
            def apply_to_neighbor(per2: str | None, resid_map: Dict[str, float]) -> Dict[str, float]:
                ...
                for m in metrics:
                    ...
                    for j, i2 in enumerate(idxs2):
                        ...
                        head2.append((j, cap2))
                    finite2 = [(j, c) for j, c in head2 if c != float('inf') and c > 1e-9]
                    if finite2:
            ...
            if dir_choice in ("prev", "next", "auto"):
                if dir_choice == "prev":
                    ...
                    h_prev = _headroom_for_period(fam, prev_per, residual_after_period) if prev_per else 0.0
                    h_next = _headroom_for_period(fam, next_per, residual_after_period) if next_per else 0.0
                ...
                if per2:
                    ...
                    try:
                        if dir_choice == "auto":
                            log.update({
                                "headroom_prev": round(h_prev, 6) if 'h_prev' in locals() else None,
                                "headroom_next": round(h_next, 6) if 'h_next' in locals() else None,
                                "cap_norm_prev": round(_cap_norm(prev_per), 6) if prev_per else None,
                                "cap_norm_next": round(_cap_norm(next_per), 6) if next_per else None,
                            })
                    except Exception:
                        pass
                ...
                prev_map = {m: (residual_after_period.get(m) or 0.0) * prev_ratio for m in metrics}
                next_map = {m: (residual_after_period.get(m) or 0.0) * next_ratio for m in metrics}
                ...
                if per_prev:
                    carryover_logs.append({
                        ...
                        "to_period": per_prev,
                        "metrics": app_prev,
                        "cap_norm": round(_cap_norm(per_prev), 6) if per_prev else None,
                    })
                if per_next:
                    carryover_logs.append({
                        "family": fam,
                        ...
                        "to_period": per_next,
                        "metrics": app_next,
                        "cap_norm": round(_cap_norm(per_next), 6) if per_next else None,
                    })

    # carryoverサマリ（noteへ要約文字列、詳細はcarryover_summary）
    moved_prev = sum(1 for r in (carryover_logs or []) if str(r.get("to_period","")) < str(r.get("from_period","")))
    moved_next = sum(1 for r in (carryover_logs or []) if str(r.get("to_period","")) > str(r.get("from_period","")))
