...
def _load_inputs(paths: List[str]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    if len(paths) != 2:
        raise ValueError("-i/--inputs には2ファイルを指定してください（aggregate と sku_week）")
...
def _sum3(d: Dict[str, float]) -> float:
    return float(d.get("demand", 0) or 0) + float(d.get("supply", 0) or 0) + float(d.get("backlog", 0) or 0)
...
def main() -> None:
    ...
    ap.add_argument("--version", dest="version_id", default=None, help="任意のversion_id")
    ap.add_argument("--tol-abs", dest="tol_abs", type=float, default=1e-6, help="絶対許容誤差")
    ap.add_argument("--tol-rel", dest="tol_rel", type=float, default=1e-6, help="相対許容誤差")
    ap.add_argument("--weeks", dest="weeks_per_period", type=int, default=4, help="月→週の週数ヒント（キー整形補助）")
    ...
    det_map: DefaultDict[Tuple[str, str], Dict[str, float]] = _dd(lambda: {"demand": 0.0, "supply": 0.0, "backlog": 0.0})
    for r in det_rows:
    ...
    for fam_per in keys:
        ...
        for m in metrics:
            ...
            row[f"ok_{m}"] = ok_m
        row["ok"] = ok_all
        if (cutover_month and per == cutover_month) or (cutover_iso and per == cutover_iso):
    ...
    for r in boundary_rows:
        for m in metrics:
            boundary_max_abs[m] = max(boundary_max_abs[m], abs(float(r.get(f"delta_{m}", 0) or 0)))
