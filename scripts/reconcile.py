...
def main() -> None:
    ...
    ap.add_argument("--blend-split-next", dest="blend_split_next", type=float, default=None, help="anchor=blend時のpost配分率(0..1)。未指定時は週別スピル×重みから動的算定")
    ap.add_argument("--blend-weight-mode", dest="blend_weight_mode", choices=["tri", "lin", "quad"], default="tri", help="blend時の重み関数: tri=三角, lin=線形, quad=二次（近接を強調）")
    args = ap.parse_args()
    ...
    use_v2 = bool(args.anchor_policy) and str(args.anchor_policy).upper() in ("DET_NEAR", "DET-NEAR")
    ...
    if use_v2 and cutover_month:
        pre_weeks = [w for w in weeks if (len(w) >= 7 and w[4] == "-" and w[:7] < cutover_month)]
        at_weeks = [w for w in weeks if (len(w) >= 7 and w[4] == "-" and w[:7] == cutover_month)]
        post_weeks = [w for w in weeks if (len(w) >= 7 and w[4] == "-" and w[:7] > cutover_month)]
        pol = str(args.anchor_policy).upper()
        if pol in ("DET_NEAR", "DET-NEAR"):
            # 1) pre: 通常前進
            pre_adj, pre_rep, pre_slack, pre_spill = _adjust_segment(pre_weeks, load_by_week, cap_w, start_slack=0.0, start_spill=0.0, mode="forward")
            # 2) at: det_near（スピルは区間外へ=postへ）
            at_adj, at_rep, at_slack, at_spill_to_post = _adjust_segment(at_weeks, load_by_week, cap_w, start_slack=pre_slack, start_spill=pre_spill, mode="det_near")
            # 3) post: 通常前進（受け取るspillに at_spill を加算）
            post_adj, post_rep, _post_slack, _post_spill = _adjust_segment(post_weeks, load_by_week, cap_w, start_slack=at_slack, start_spill=at_spill_to_post, mode="forward")
            # マージ
            ...
            pre1_adj, pre1_rep, pre1_slack, pre1_spill = _adjust_segment(pre_weeks, load_by_week, cap_w, start_slack=0.0, start_spill=0.0, mode="forward")
            # 2) at: agg_far（スピルは区間外へ=preへ）
            at_adj, at_rep, at_slack, at_spill_to_pre = _adjust_segment(at_weeks, load_by_week, cap_w, start_slack=pre1_slack, start_spill=pre1_spill, mode="agg_far")
            # 3) pre（二次）: atからのスピルを受けて再計算
            pre2_adj, pre2_rep, _pre2_slack, _pre2_spill = _adjust_segment(pre_weeks, load_by_week, cap_w, start_slack=0.0, start_spill=at_spill_to_pre, mode="forward")
            # 4) post: 通常前進（atのスラックを引継ぎ、spillは0）
            post_adj, post_rep, _post_slack, _post_spill = _adjust_segment(post_weeks, load_by_week, cap_w, start_slack=at_slack, start_spill=0.0, mode="forward")
            # マージ（preは二次結果を採用）
            ...
            pre1_adj, pre1_rep, pre1_slack, pre1_spill = _adjust_segment(
                pre_weeks, load_by_week, cap_w, start_slack=0.0, start_spill=0.0, mode="forward"
            )
            # 2) at（吐き出しを後でpre/post に分割）
            at_adj, at_rep, at_slack, at_spill_total = _adjust_segment(
                at_weeks, load_by_week, cap_w, start_slack=pre1_slack, start_spill=pre1_spill, mode="blend"
            )
            ...
            if args.blend_split_next is not None:
                ...
                win_w = int(math.ceil(float(args.recon_window_days) / 7.0)) if args.recon_window_days else max(1, n_at // 2)
                # 週キー→インデックスのマップ
            ...
            pre2_adj, pre2_rep, _pre2_slack, _pre2_spill = _adjust_segment(pre_weeks, load_by_week, cap_w, start_slack=0.0, start_spill=spill_prev, mode="forward")
            # 5) post: at のスラック + spill_next を受けて前進
            post_adj, post_rep, _post_slack, _post_spill = _adjust_segment(post_weeks, load_by_week, cap_w, start_slack=at_slack, start_spill=spill_next, mode="forward")
            # マージ（preは二次結果）
            for d in (pre2_adj, at_adj, post_adj):
                adj_by_week.update(d)
            week_report.extend(pre2_rep + at_rep + post_rep)
    ...
    if cutover_month:
        ...
        at_weeks = [r for r in week_report if str(r.get("week",""))[:7] == cutover_month]
        ...
        win_weeks = int(math.ceil(float(args.recon_window_days)/7.0)) if args.recon_window_days else 0
        ...
        "boundary_summary": (
            {
                ...
                "weeks": len([r for r in week_report if str(r.get("week",""))[:7] == cutover_month]),
                "pre_weeks": len([r for r in week_report if str(r.get("zone")) == "pre"]),
                "post_weeks": len([r for r in week_report if str(r.get("zone")) == "post"]),
            }
            if cutover_month
            else None
        ),
