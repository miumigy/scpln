...
def _service_level_z(p: float) -> float:
    """Map service level to z-value.
    - p <= 0 -> 0 (安全在庫なし)
    - 0 < p < 1 -> 正規分布の逆関数
    - p >= 1 -> 上限クリップ（実務上は発生しないが保険で6σ）
    """
    if p is None:
        return 0.0
    if p <= 0.0:
...
class FactoryNode(BaseNode):
    ...
    )  # Added for finished goods inventory management
...
class SupplyChainSimulator:
    def __init__(self, sim_input: SimulationInput):
        ...
        self.in_transit_orders = defaultdict(
            list
        )  # {arrival_day: [(item, qty, dest, from)]}
    ...
    def run(self):
        for day in range(self.input.planning_horizon):
            ...
            received_orders_today = defaultdict(
                list
            )  # New: To store orders received today
            in_transit_at_start_of_day = deepcopy(self.in_transit_orders)  # For logging
            logging.debug(
                f"Day {day}: in_transit_orders at start of day: {self.in_transit_orders}"
            )
            ...
            if day in self.pending_shipments:
                ...
                dest_incoming_today = defaultdict(
                    float
                )  # per destination node per day (for storage capacity)
            ...
            if day in self.in_transit_orders:
                for item, qty, dest_node_name, src_node_name in self.in_transit_orders[
                    day
                ]:
                    logging.debug(
                        f"Day {day}: Receiving incoming shipment: Item {item}, Qty {qty}, Dest {dest_node_name}"
                    )
                    self.stock[dest_node_name][item] += qty
                    daily_events[f"{dest_node_name}_{item}"]["incoming"] += qty
                    received_orders_today[dest_node_name].append(
                        (item, qty, src_node_name)
                    )  # New: Store received orders
                del self.in_transit_orders[day]
            logging.debug(
                f"Day {day}: in_transit_orders after receiving: {self.in_transit_orders}"
            )
            ...
            for node_name in self.node_order:
                ...
                if isinstance(current_node, (StoreNode, WarehouseNode)):
                    ...
                    for item_name in items_to_manage:
                        ...
                        order_up_to = z * demand_std * math.sqrt(
                            replenishment_lt
                        ) + demand_mean * (replenishment_lt + 1)
                        ...
                        if qty_to_order > 0:
                            ...
                            logging.debug(
                                f"Day {day}: Replenishment order of {qty_to_order} for {item_name} from {node_name} to {parent_name}. Placing order via _place_order."
                            )
                            self._place_order(
                ...
                if isinstance(current_node, FactoryNode):
                    ...
                    for item_name in current_node.producible_products:
                        profile = self.factory_demand_profiles.get(node_name, {}).get(
                            item_name
                        )
                        if not profile:
                            continue
                        ...
                        order_up_to = z * profile["std_dev"] * math.sqrt(
                            current_node.lead_time
                        ) + profile["mean"] * (current_node.lead_time + 1)
                        production_needed = max(0, math.ceil(order_up_to - inv_pos))
                        # Target production quantity before material check
                        target_prod = (
                            production_needed
                            if getattr(
                                current_node, "allow_production_over_capacity", True
                            )
                            else min(
                                production_needed, current_node.production_capacity
                            )
                        )
                        ...
                        if target_prod > 0:
                            ...
                            for bom in self.products[item_name].assembly_bom:
                                ...
                                if bom.quantity_per > 0:
                                    max_by_components = min(
                                        max_by_components,
                                        math.floor(avail / bom.quantity_per),
                                    )
                            producible_qty = max(
                                0,
                                min(
                                    ...
                                    int(
                                        max_by_components
                                        if max_by_components != float("inf")
                                        else target_prod
                                    ),
                                ),
                            )
                        ...
                        if producible_qty > 0:
                            logging.debug(
                                f"Day {day}: Production order of {producible_qty} for {item_name} at {node_name}"
                            )
                            for bom in self.products[item_name].assembly_bom:
                                consumed = bom.quantity_per * producible_qty
                                self.stock[node_name][bom.item_name] -= consumed
                                daily_events[f"{node_name}_{bom.item_name}"][
                                    "consumption"
                                ] += consumed
                            self.production_orders[day + current_node.lead_time].append(
                                (item_name, producible_qty, node_name)
                            )
                    ...
                    for item_name in {
                        c.item_name
                        for p in current_node.producible_products
                        for c in self.products[p].assembly_bom
                    }:
        ...
        for key, data in events.items():
            if "sales" in data:
                ...
                if self.nodes_map[node_name].node_type == "store":
                    pl["revenue"] += (
                        data["sales"] * self.products[item_name].sales_price
                    )
        ...
        for key, data in events.items():
            ...
            if "_" in key:
                ...
                if node and "produced" in data and isinstance(node, FactoryNode):
                    ...
                    pl["flow_costs"]["production_variable"] += (
                        node.production_cost_variable * qty_prod
                    )
            ...
            if key.startswith("transport:"):
                ...
                if isinstance(supplier, MaterialNode) and isinstance(dest, FactoryNode):
                    pl["material_cost"] += (
                        supplier.material_cost.get(shipped_item, 0) * qty
                    )
                ...
                if link_key not in links_with_fixed_cost_applied:
                    if isinstance(supplier, MaterialNode) and isinstance(
                        dest, FactoryNode
                    ):
