...
- `scripts/allocate.py` / `scripts/mrp.py` / `scripts/reconcile.py` / `scripts/reconcile_levels.py` / `scripts/anchor_adjust.py`
 で `--calendar` を優先し、`planning_calendar.json` がない場合のみ `--weeks` で等分フォールバック。
