...
本書は Planning Hub のUX/機能ロードマップ、役割、導入段階を整理した設計ドキュメントです。操作手順は `docs/TUTORIAL-JA.md`、集約↔詳細アルゴリズムは `docs/AGG_DET_RECONCILIATION_JA.md`、API概要は `docs/API-OVERVIEW-JA.md` を参照し、本書では「どの体験をいつ提供するか」に専念します。
...
- [x] P-14 旧画面クローズ & ドキュメント整理（完了）
  - 進捗: 2025-09-12 旧UI `/ui/planning` を完全削除。ナビ/リンク/テンプレ/テストをHubへ統合。
  - 進捗: 2025-09-12 docs/README/TUTORIAL/API-OVERVIEW/TERMS をHub前提に更新。
