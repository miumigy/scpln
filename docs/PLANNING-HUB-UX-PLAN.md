...
#### P1 併存期（Hub導入 + API統一）
- [ ] UXワイヤー（Hub/タブ/右ペイン）作成・承認
- [ ] Run API仕様ドラフト（エンドポイント/ペイロード/状態機械）
- [ ] 既存3入口→Run APIアダプタ実装（内部経路統一）
- [ ] 差分/KPIプレビュー（MVP: 在庫・発注・SL）
- [ ] Planning Hub 初版（設定/パイプライン/実行タブ）
- [ ] 計測イベント仕込み（到達時間/クリック/離脱）
- [ ] README/ヘルプ更新（新動線の案内）

#### P2 統合期（画面統合 + 非推奨化）
- [ ] パイプライン/ジョブ画面機能をHubへ統合
- [ ] 既存リンクの非推奨バナー・Hub内セクションへの誘導
- [ ] 履歴・固定リンクの強化（フィルタ/再実行/共有）

#### P3 整理期（旧入口撤去 + ドキュメント）
- [ ] Home実行エントリ撤去 + 一時リダイレクト
- [ ] 用語統一の完了（UI/README/APIドキュメント）
- [ ] チュートリアル/ハンズオン更新
